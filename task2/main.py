...
if __name__ == '__main__':
    ...
    if should_run_rf:
        classifier = RandomForestClassifier(num_trees=100, num_features=int(math.sqrt(num_features)), batch_size=len(train), test_count=10, test_divide=0.2, max_depth=12)
        if is_test:
        ...
        classifier = DecisionTree(batch_size=len(train), test_count=3, test_divide=0.2, max_depth=12)
        if is_test:
            classifier.fit_test_predict(train)
        else:
            predictions = classifier.fit_predict(train, task)
